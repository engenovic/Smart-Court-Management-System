            ...
            <ul class="nav navbar-nav navbar-right">
			    ...
			    <li class="dropdown">
	        		...
	        		<ul class="dropdown-menu">
						...
						<li class="m_2"><a href="#"><i class="fa fa-users"></i> Manage Clients</a></li>
						<li class="m_2"><a href="#"><i class="fa fa-user"></i> Manage Magistrates</a></li>
						<li class="divider"></li>
                      ...
                      <div class="panel-body">
                        <form action="case.php" method="post">
                          <div class="alert alert-info">
                              Please fill case details here
                          </div>
                          <hr>
                          <label>Enter Case Title : </label>
                          <input type="text" name="title" class="form-control1 control3">
                          <label>Select Defendant :  </label>
                          <select name="defend" class="form-control mt-3">
                            <?php
                              foreach($defends as $defend) {
                                echo "<option value=\"{$defend['clientID']}\" >{$defend['fname']} {$defend['lname']}</option>";
                              }
                              ?>

                                                  </select>
                                                  <label>Select Accuser :  </label>
                                                  <select name="accuse" class="form-control mt-3">
                                                    <?php
                                                      foreach($accuses as $ac) {
                                                        echo "<option value=\"{$ac['clientID']}\" >{$ac['fname']} {$ac['lname']}</option>";
                                                      }
                                                      ?>
                                                                          </select>
                          <label>Enter Case Description : </label>
                          <textarea rows="6" name="desc" class="form-control1 control2"></textarea>
                          <hr>
